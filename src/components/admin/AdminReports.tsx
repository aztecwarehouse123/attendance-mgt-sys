...
type DailyData = { date: string; punches: number; amount: number; entries?: any[] };
  ...
  const generateChartData = () => {
    ...
    filteredUsers.forEach(user => {
      ...
      Object.entries(logByDay).forEach(([dateKey, log]) => {
        ...
        if (!dailyData[dateKey]) dailyData[dateKey] = { date: dateKey, punches: 0, amount: 0 };
    ...
    const filledChartData = allDates.map(date =>
      chartDataMap[date] || { date, punches: 0, amount: 0 }
    );
  ...
  const getTotalStats = () => {
    ...
    filteredUsers.forEach(user => {
      ...
      allDates.forEach(dateStr => {
        // Convert 'MMM dd' back to 'yyyy-MM-dd' for lookup
        const dateObj = new Date(`${dateStr} ${end.getFullYear()}`);
        const dateKey = format(dateObj, 'yyyy-MM-dd');
        const log = logByDay[dateKey] || [];
        const punches = Math.floor(log.length / 2);
        const hours = calculateTotalHoursForDay(log);
    ...
    return { totalPunches, totalAmount, uniqueUsers };
  };
